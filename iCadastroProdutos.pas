...
If Button = nbDelete then
  begin
    if DataModule1.tbPedidoProduto.Locate('Id_Produto',DBGrid1.Fields[0].AsInteger,[]) = true then
      begin
        showmessage('Não é possível excluir este produto porque ele já está em um pedido realizado.');
        abort;
      end;
    If MessageDlg('Confirma a exclusão do registro atual?', mtWarning, [mbYes, mbNo], 0) = mrno  Then
      Abort;
  end;
end;
